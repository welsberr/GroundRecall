...
This gives the paper a concrete mechanism for robust contradiction tracking: disagreement remains historically visible, and resolution is represented as additional review state rather than silent deletion or averaging. The current implementation does not infer semantic contradictions automatically; it relies on explicit contradiction links created by import/review workflows.
