...
| Benchmark literature now provides concrete targets for evaluating long-term memory and graph-RAG behavior. | Bibliography/source analysis; future-work status | LongMemEval, LoCoMo, MemoryAgentBench, LoCoMo-Plus, GraphRAG, GraphRAG survey, GraphRAG-Bench, and Agentic GraphRAG entries in `docs/preprint/memory-layer-citegeist-export.bib`; limitations in `docs/preprint/preprint-draft.md` | Supported as evaluation-context framing and future evaluation target selection. | GroundRecall has not yet been run against those benchmarks. |
| Persistent AI memory and RAG systems raise privacy, memory-injection, memory-extraction, and retrieval-authorization concerns. | Bibliography/source analysis | Agent-Memory Protocol, CAMS, and Permission-Aware RAG entries in `docs/preprint/memory-layer-citegeist-export.bib` | Supported for problem-framing and comparison. | The coverage is initial, not a complete privacy-leakage or authorization survey. |
...
| ClaimWright is one suitable policy framework, not a universal stance. | Implemented code; design argument | ClaimWright repository policy substrate; `docs/preprint/preprint-draft.md` policy-pluralism section; GroundRecall policy-plugin adapter | Supported as an example policy stance and adapter target. | ClaimWright remains optional policy content under GroundRecall's policy-plugin contract, not a mandatory dependency or externally validated policy authority. |
