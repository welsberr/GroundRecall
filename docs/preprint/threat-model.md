...
| Restricted material is treated as ordinary confidential material | Roadmap requires `restricted` to be treated as a restriction marker, not a release-level alias; restricted federation should fail closed unless an explicit scoped policy authorizes transfer or derivative release. | Current prototype needs restriction-aware federation hardening before restricted, compartmented, HR, legal, incident, source-protected, or export-controlled material should federate. |
| Catalog discovery leaks protected existence metadata | Roadmap requires restricted scopes to be absent from catalogs by default, including opaque/aggregate counts, topics, time coverage, and membership signals. | Sanitized discovery metadata needs explicit policy design and tests. |
