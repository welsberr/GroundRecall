...
| Restricted material is treated as ordinary confidential material | `restricted` is no longer normalized to `confidential`; restriction markers are separate from release level; restricted federation fails closed unless explicit policy metadata and receiver-side acceptance allow it. | Current implementation is metadata-based; first-class restriction schema fields and external IAM/DLP integration remain future work. |
| Catalog discovery leaks protected existence metadata | Restricted scopes and restricted dependent records are omitted from federation catalogs by default, including opaque/aggregate counts, topics, time coverage, and membership signals. | Sanitized discovery metadata needs explicit policy design and tests beyond the default-deny behavior. |
