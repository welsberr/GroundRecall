...
def consume_handoff_promotion_action(store_dir: str | Path, handoff_id: str, *, action_id: str, requester_subject_id: str, project: str, promotion_target: str, confirm: bool, policy_provider: PolicyDecisionProvider, realm_id: str = "", maximum_release_level: str = "private", idempotency_key: str = "", provenance: dict[str, Any] | None = None, promotion_executor: Callable[[dict[str, Any]], dict[str, Any]] | None = None, executor_name: str = "", executor_registry: dict[str, Callable[[dict[str, Any]], dict[str, Any]]] | None = None, executor_timeout_seconds: float = 5.0, max_executor_result_bytes: int = 8192) -> HandoffEvent:
    ...
    with _HANDOFF_LOCK:
        ...
        if promotion_executor is not None:
            if executor_timeout_seconds <= 0 or executor_timeout_seconds > 300:
                raise ValueError("executor timeout must be between 0 and 300 seconds")
            try:
                with ThreadPoolExecutor(max_workers=1) as pool:
                    result = pool.submit(promotion_executor, {"handoff_id": handoff_id, "action_id": action_id, "project": project, "promotion_target": promotion_target, "realm_id": realm_id}).result(timeout=executor_timeout_seconds)
            except (FutureTimeoutError, Exception) as exc:
                effect = {"canonical_effect": "none", "executor_name": executor_name, "executor_error": type(exc).__name__}
                result = None
            if result is not None and (not isinstance(result, dict) or len(json.dumps(result, separators=(",", ":"))) > max_executor_result_bytes):
                effect = {"canonical_effect": "none", "executor_name": executor_name, "executor_error": "invalid_result"}
                result = None
...
def block_handoff(store_dir: str | Path, handoff_id: str, *, subject_id: str, host_id: str, project: str, lease_id: str, reason: str = "", evidence_ref: str = "", policy_provider: PolicyDecisionProvider | None = None, realm_id: str = "", maximum_release_level: str = "private", expected_status: str = "executing", idempotency_key: str = "", provenance: dict[str, Any] | None = None) -> HandoffResult:
    """Block an accepted/executing handoff with a lease-bound reason."""
    if not reason.strip() and not evidence_ref.strip():
        raise ValueError("handoff block requires reason or evidence_ref")
    with _HANDOFF_LOCK:
        item = get_handoff(store_dir, handoff_id, subject_id=subject_id, realm_id=realm_id, maximum_release_level=maximum_release_level)
        if item is None or item.project != project:
            raise PermissionError("handoff block scope does not match")
        existing = _event_idempotent(store_dir, handoff_id, idempotency_key, subject_id=item.subject_id, realm_id=item.realm_id)
        policy = _handoff_policy(policy_provider, action="handoff_block", handoff=item, status="blocked")
        if policy.decision in {"deny", "hard_gate"}:
            raise PermissionError("policy blocked handoff block transition")
        ...
        if expected_status not in {"accepted", "executing"} or item.status != expected_status:
            raise ValueError(f"handoff status conflict: expected accepted or executing, found {item.status}")
        if not lease_id or lease_id != item.lease_id or not _lease_is_active(item) or item.lease_subject_id != subject_id or item.lease_host_id != host_id:
            raise PermissionError("handoff block requires active lease owner")
        now = _now(); item.status = "blocked"; item.updated_at = now
        event = HandoffEvent(event_id=f"event-{uuid.uuid4().hex[:16]}", event_type="status", handoff_id=item.handoff_id, task_id=item.task_id, subject_id=item.subject_id, realm_id=item.realm_id, release_level=item.release_level, status="blocked", outcome=reason, result_ref=evidence_ref, lease_id=lease_id, lease_subject_id=subject_id, lease_host_id=host_id, lease_expires_at=item.lease_expires_at, provenance={**dict(provenance or {}), "blocked_by_host": host_id}, idempotency_key=idempotency_key, created_at=now)
        _persist_mutation(store_dir, item, event)
        return HandoffResult(handoff=item, policy_decision=policy, lease_id=lease_id, lease_expires_at=item.lease_expires_at)


def unblock_handoff(store_dir: str | Path, handoff_id: str, *, subject_id: str, host_id: str, project: str, lease_id: str, resolution: str = "", evidence_ref: str = "", policy_provider: PolicyDecisionProvider | None = None, realm_id: str = "", maximum_release_level: str = "private", expected_status: str = "blocked", idempotency_key: str = "", provenance: dict[str, Any] | None = None) -> HandoffResult:
    """Resolve a blocked handoff back to accepted under its active lease."""
    if not resolution.strip() and not evidence_ref.strip():
        raise ValueError("handoff unblock requires resolution or evidence_ref")
    with _HANDOFF_LOCK:
        item = get_handoff(store_dir, handoff_id, subject_id=subject_id, realm_id=realm_id, maximum_release_level=maximum_release_level)
        if item is None or item.project != project:
            raise PermissionError("handoff unblock scope does not match")
        existing = _event_idempotent(store_dir, handoff_id, idempotency_key, subject_id=item.subject_id, realm_id=item.realm_id)
        policy = _handoff_policy(policy_provider, action="handoff_unblock", handoff=item, status="accepted")
        if policy.decision in {"deny", "hard_gate"}:
            raise PermissionError("policy blocked handoff unblock transition")
        if existing is not None and existing.event_type == "status" and existing.status == "accepted":
            return HandoffResult(handoff=item, policy_decision=policy, lease_id=item.lease_id, lease_expires_at=item.lease_expires_at)
        if item.status != expected_status or expected_status != "blocked":
            raise ValueError(f"handoff status conflict: expected blocked, found {item.status}")
        if not lease_id or lease_id != item.lease_id or not _lease_is_active(item) or item.lease_subject_id != subject_id or item.lease_host_id != host_id:
            raise PermissionError("handoff unblock requires active lease owner")
        now = _now(); item.status = "accepted"; item.updated_at = now
