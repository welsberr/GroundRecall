...
POLICY_COVERAGE_ENTRIES: tuple[PolicyCoverageEntry, ...] = (
    ...
    PolicyCoverageEntry(
        ...
        status="future",
        durable_memory_change=True,
        enforcement=["contribution state transitions validate lifecycle order and require a reviewer/rationale", "ClaimWright policy preflight is planned for IF-03"],
        audit=["append-only review receipt is produced by the transition helper"],
        tests=["tests/test_institutional_federation.py"],
        caveat="The lifecycle helper is implemented, but policy-gated store application is not yet wired.",
    ),
    PolicyCoverageEntry(
        ...
        status="future",
        durable_memory_change=True,
        enforcement=["scope/work and lifecycle record models are implemented", "policy-plugin enforcement is planned for IF-03"],
        audit=[],
        tests=["tests/test_institutional_federation.py"],
        caveat="Direct store save methods remain low-level APIs until institutional policy preflight is integrated.",
    ),
    PolicyCoverageEntry(
