...
POLICY_COVERAGE_ENTRIES: tuple[PolicyCoverageEntry, ...] = (
    ...
    PolicyCoverageEntry(
        ...
        status="partial",
        durable_memory_change=True,
        enforcement=["dry-run by default", "sensitive/private/no-export records are screened before candidate generation"],
        audit=["write_summary and relation_examples returned; no policy-plugin gate or durable audit event yet"],
        tests=["tests/test_graph_augment.py", "tests/test_graph_maintenance.py", "tests/test_export_guardrails.py"],
        caveat="Candidate writes should receive an optional policy-plugin preflight before the preprint claims complete mutation-path coverage.",
    ),
    PolicyCoverageEntry(
        ...
        status="partial",
        durable_memory_change=True,
        enforcement=["dry-run unless --apply is supplied", "bounded profiles, state files, locks, stale-lock recovery", "delegates sensitive-record screening to graph backfill"],
        audit=["run history stored in maintenance state when state advances; no policy-plugin gate yet"],
        tests=["tests/test_graph_maintenance.py"],
        caveat="Scheduled graph-maintenance writes should receive an optional policy-plugin preflight and durable denied-write audit.",
    ),
    PolicyCoverageEntry(
        route_id="cli.import",
        surface="cli",
