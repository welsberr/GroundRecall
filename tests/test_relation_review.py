...
def _seed_relation_store(store_dir: Path) -> GroundRecallStore:
    ...
    store.save_observation(
        ObservationRecord(
            ...
            role="source_excerpt",
            text="The source explicitly links concept A to concept B in a short reviewable passage.",
            provenance=ProvenanceRecord(support_kind="derived_from_page", grounding_status="grounded"),
            current_status="reviewed",
        )
    )
    store.save_claim(
        ClaimRecord(
            claim_id="claim_1",
            claim_text="Concept A supports concept B in the fixture.",
            source_observation_ids=["obs_1"],
            concept_ids=["concept::evo-edu-a"],
            provenance=ProvenanceRecord(support_kind="derived_from_page", grounding_status="grounded"),
            current_status="reviewed",
        )
    )
...
def test_list_relation_review_batch_filters_and_sorts(tmp_path: Path) -> None:
    ...
    assert payload["relations"][0]["evidence_previews"][0]["observation_text"].startswith("The source explicitly links")
    assert payload["relations"][0]["evidence_previews"][0]["claim_previews"][0]["claim_id"] == "claim_1"
...
def test_apply_relation_review_batch_updates_relation_candidate_and_audit(tmp_path: Path) -> None:
    ...
    replacement_id = payload["applied"][0]["replacement_relation_id"]
    replacement = store.get_relation(replacement_id)
    review = store.get_review_candidate("rq_rel_review_me")
    replacement_review = store.get_review_candidate(f"rq_{replacement_id}")
    promotions = store.list_promotions()
    assert payload["applied_count"] == 1
    assert payload["skipped_count"] == 0
    assert relation is not None
    assert relation.current_status == "rejected"
    assert relation.relation_type == "mentions_topic"
    assert replacement is not None
    assert replacement.current_status == "reviewed"
    assert replacement.relation_type == "supports"
    assert review is not None
    assert review.current_status == "rejected"
    assert "Evidence supports" in review.rationale
    assert replacement_review is not None
    assert replacement_review.current_status == "reviewed"
    assert "relation_retyped" in replacement_review.finding_codes
